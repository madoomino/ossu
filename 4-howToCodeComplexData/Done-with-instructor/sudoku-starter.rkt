...
;; Board -> Board of false
;; produces a solution for bd; or false if no solution for db
;; Assume: bd is valid
...
(define (solved? bd)
  (andmap (lambda (sq) (not (false? sq))) bd))

;; Board -> (listof Board)
;; prduces the next valid (listof Board)
;; it fills the 1st empty square with Natural[1,9], and keeps only valid boards
...
;; Board -> Pos
;; finds the first blank square and returns its position
;; !!!
(define (find-blank bd) 0)

;; Pos Board -> (listof Board)
;; produces 9 boards, with blank square filled with Natural[1,9]
;; !!!
(define (fill-with-1-9 pos bd) empty)

;; (listof Board) -> (listof Board)
;; produces a list containing only valid boards
;; !!!
