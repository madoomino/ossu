...
(* Do not make chaanges to this code except where you see comments containing
   the word CHAANGE. *)
...
(* CHANGE_DONE: Add function preprocess_prog of type geom_exp -> geom_exp *)
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  | Shift(dx, dy, sub_e) => 
	  	case eval_prog(sub_e,env) of
		NoPoints => NoPoints
		| Point (x, y) => Point (x + dx, y + dy)
		| Line (m, b) => Line (m, b + dy - m * dx)
		| VerticalLine x => VerticalLine (x + dx)
		| LineSegment (x1,y1,x2,y2) => preprocess_prog(LineSegment(x1+dx, y1+dy, x2+dx, y2+dy))
		| _ => sub_e
(* CHANGE_DONE: Add a case for Shift expressions *)
